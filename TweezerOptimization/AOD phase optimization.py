...
class Tones:
    ...
    def visualize(self, x_start, x_end):
        ...
        axs[0].bar(self.freqs, self.realamps, color = colors, width = 0.2)
...
plot_yes = True
Ntweezer = 20
mod_depth  = PI/4/np.sqrt(Ntweezer)

center_f = 100
delta_f = 1
start_f = center_f - (Ntweezer-1)/2 * delta_f
end_f = center_f + (Ntweezer-1)/2 * delta_f
# f_list = np.arange(start_f,end_f+delta_f, delta_f)+0.0*(-1)**np.arange(Ntweezer)
...
phi_list = np.cumsum(np.append([0],delta_phi_list)) + np.random.random(Ntweezer) /3
...
print(f"phi opt init cost func = {thirdtone_cost_func_6(phi_list)}")
...
if optimize_yes:
    ...
    optimized = scipy.optimize.minimize(thirdtone_cost_func_6, phi_list, args=(),\
