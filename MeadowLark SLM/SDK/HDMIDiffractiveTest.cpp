...
int main()
{
	...
	ITLCameraSDK sdk = TLCameraSDK.OpenTLCameraSDK(); //Create Instance of the TL Camera SDK 
  	...
  	cam.ExposureTime_us = 4; // in microseconds
  	cam.Arm();
	...
	int PixelsPerStripe = 128; //use a fairly high frequency pattern (a short period) to separate the 0th from the 1st order
	for (int region = 0; region < NumRegions; region++)
	{
		...
		for (int Gray = 0; Gray < NumDataPoints; Gray++)
		{
			...
			//generate a stripe
			Generate_Stripe(Image, WFC, width, height, depth, PixelValueOne, PixelValueTwo, PixelsPerStripe, RGB);
